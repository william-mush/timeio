...
/**
 * Import GeoNames cities15000.txt into PostgreSQL
 * 
 * This script imports ~33,000 cities (population 15000+) from GeoNames into the geo_cities table.
 * 
 * Usage:
 *   npx ts-node scripts/geonames/import-cities15000.ts
 */
...
// Set up Prisma with driver adapter (Prisma 7 pattern)
...
const countryNames: Record<string, string> = {
    ...
    'KR': 'South Korea', 'ID': 'Indonesia', 'NL': 'Netherlands', 'SA': 'Saudi Arabia',
    ...
    'UA': 'Ukraine', 'EG': 'Egypt', 'ZA': 'South Africa', 'NG': 'Nigeria',
    'PK': 'Pakistan', 'BD': 'Bangladesh', 'VN': 'Vietnam', 'TH': 'Thailand',
    'PH': 'Philippines', 'MY': 'Malaysia', 'SG': 'Singapore', 'HK': 'Hong Kong',
    'TW': 'Taiwan', 'AE': 'United Arab Emirates', 'IL': 'Israel', 'TR': 'Turkey',
    'AR': 'Argentina', 'CL': 'Chile', 'CO': 'Colombia', 'PE': 'Peru', 'VE': 'Venezuela',
    'NZ': 'New Zealand', 'KE': 'Kenya', 'ET': 'Ethiopia', 'TZ': 'Tanzania',
    'IR': 'Iran', 'IQ': 'Iraq', 'AF': 'Afghanistan', 'NP': 'Nepal', 'LK': 'Sri Lanka'
};
...
async function importCities() {
    ...
    if (!fs.existsSync(filePath)) {
        console.error('cities15000.txt not found!');
        process.exit(1);
    }

    console.log('Starting import from cities15000.txt...');
    console.log('Processing ~33,000 cities with population 15000+\n');
    ...
    for await (const line of rl) {
        ...
        if (batch.length >= BATCH_SIZE) {
            ...
            // Progress update every 5k records
    ...
    console.log(`\nVerified: ${count.toLocaleString()} cities now in geo_cities table`);
